...
function ProductCreate() {
  ...
  const objectValid = {
    code: Yup.string()
      ...
      .matches(
        /^PROD-\d{4}$/,
        "Mã sản phẩm nhập chưa đúng định dạng (yêu cầu : PROD - XXXX với X là các chữ số"
      ),
  ...
  const saveProduct = async (values, { setSubmitting }) => {
    try {
      const isCodeExists = await productService.checkProductCodeExists(
        values.code
      );
      if (isCodeExists) {
        toast.error("Mã sản phẩm đã tồn tại. Vui nhập mã khác.");
        setSubmitting(false);
        return;
      }
